...
def main(train, dev, output):
    ...
    **Note : il faut que les corpus soient déjà nettoyés**
    ...
    conll_train = train # corpus de train
    conll_dev = dev # corpus de dev pour tester le modèle
    model_dir = output # le dossier dans lequel on veut sauvegarder le modèle
...
if __name__=="__main__":
    train = sys.argv[1]  # corpus de train
    dev = sys.argv[2]  # corpus de dev pour tester le modèle
    output = sys.argv[3]  # le dossier dans lequel on veut sauvegarder le modèle
    main(train, dev, output)