...
public class ChainCRFTagger<V, E, L> implements Serializable {
	...
	public List<Pair<List<L>, Double>> getTopKLabelSequencesAndScores(InstanceSequence<V, E, L> s, int k) {
		...
		List<Pair<List<L>, Double>> sentences = new ArrayList<Pair<List<L>, Double>>(k);
	...
	public static class Factory<V, E, F, L> {
		...
		public Factory(FeatureExtractor<V, F> vertexExtractor, FeatureExtractor<E, F> edgeExtractor,
		...
		public ChainCRFTagger<V, E, L> trainTagger(List<? extends LabeledInstanceSequence<V, E, L>> trainingData) {
			...
			DifferentiableFunction objective = new CRFObjectiveFunction<V, E, F, L>(trainingData,
					encoding, vertexExtractor, edgeExtractor, sigma);
			LBFGSMinimizer minimizer = new LBFGSMinimizer(iterations);
			Logger.startTrack("Training with LBFGS");
			double[] w = minimizer.minimize(objective,
					DoubleArrays.constantArray(0.0, encoding.getNumFeatures()*encoding.getNumLabels()), 1e-4, true);
			Logger.endTrack();
			return new ChainCRFTagger<V, E, L>(encoding, new Inference<V, E, F, L>(encoding, vertexExtractor, edgeExtractor), w);
		}
		...
		private Encoding<F, L> buildEncoding(List<? extends LabeledInstanceSequence<V, E, L>> trainingData) {
			...
			for (LabeledInstanceSequence<V, E, L> s : trainingData) {
				for (int i=0; i<s.getSequenceLength(); i++) {
					featureIndexer.addAll(vertexExtractor.extractFeatures(s.getVertexInstance(i)).keySet());
					if (i > 0) {
						for (int l=0; l<labelIndexer.size(); l++) {
							featureIndexer.addAll(edgeExtractor.extractFeatures(s.getEdgeInstance(i, labelIndexer.getObject(l))).keySet());
						}
					}
				}
			}
